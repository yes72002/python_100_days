...
class DataManager:
    ...
    def get_users(self):
        response = requests.get(url=sheet_users_url, headers=self.headers)
        # print("response.status_code =", response.status_code) # 200
        data = response.json()
        print(data)
    ...
    def edit_user(self, user_first_name, user_last_name, user_email, id):
        sheet_edit_url = f"{sheet_users_url}/{id}"
        edit_data = {
            "user":{
